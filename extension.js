...
/**
 @brief 从某个HTML文件读取能被Webview加载的HTML内容
 @param {*} context 上下文
...
function getWebViewContent(context, templatePath, panel) {
    ...
    const html = htmlIndexPath.replace(/(<link.+?href="|<script.+?src="|<img.+?src=")(.+?)"/g, (_, $1, $2) => {
...
function generConfig(isUpdate, mingwpath, qtdir, qtkitdir, vcvarsall) {
    ...
    if (isUpdate || !fs.existsSync(cCppPropertiesFilePath) || !fs.existsSync(launchFilePath) || !fs.existsSync(settingsFilePath) || !fs.existsSync(tasksFilePath) || !fs.existsSync(debugFilePath) || !fs.existsSync(releaseFilePath)) {
...
function activate(context) {
    ...
    if (mingwpath === undefined || mingwpath.trim().length === 0
        ...
        panel.webview.onDidReceiveMessage(message => {
            //如果参数设置界面加载完成，则显示已设置的参数
            if (message.command === 'initfinished') {
                const mingwpath = vscode.workspace.getConfiguration().get('qtconfig.mingwpath')
                const qtdir = vscode.workspace.getConfiguration().get('qtconfig.qtdir')
                const qtkitdir = vscode.workspace.getConfiguration().get('qtconfig.qtkitdir')
                const vsvarsall = vscode.workspace.getConfiguration().get('qtconfig.vcvarsall')
            ...
            //如果是界面参数设置完成，则写入设置的参数
                ...
                if (mingwIncludePath.length !== 0 && msvcIncludePath.length !== 0)
                    ...
                    //生成配置文件
                    generConfig(true, mingwpath, qtdir, mingwIncludePath, vcvarsall)
                else if (msvcIncludePath.length !== 0)
                    //生成配置文件
                    generConfig(true, mingwpath, qtdir, msvcIncludePath, vcvarsall)
                //关闭 webview
                //panel.dispose()
            }
        }, undefined, context.subscriptions)
    } else {
        generConfig(false, mingwpath, qtdir, qtkitdir, vcvarsall)
    }
    /*
